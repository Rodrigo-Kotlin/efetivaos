-- 13F: Corrigir EXTRACT(EASECOND ...) -> EXTRACT(EPOCH ...) em get_crm_pipeline_analytics
-- Apenas a expressão de avg_duration_days é alterada; assinatura, grants e search_path preservados.
...
-- Reaplicar grants padrão DEC-031 (postgres + service_role + authenticated; sem anon/PUBLIC)
REVOKE EXECUTE ON FUNCTION public.get_crm_pipeline_analytics(uuid, date, date, uuid) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.get_crm_pipeline_analytics(uuid, date, date, uuid) TO authenticated;