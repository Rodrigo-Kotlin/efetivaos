...
BEGIN
  ...
  v_result := jsonb_build_object(
    ...
    'income_statement', (
      SELECT jsonb_build_object(
        'revenue', COALESCE(SUM(CASE WHEN row_code = 'RB' THEN amount ELSE 0 END), 0),
        'revenue_deductions', COALESCE(SUM(CASE WHEN row_code = 'DR' THEN amount ELSE 0 END), 0),
        'net_revenue', COALESCE(SUM(CASE WHEN row_code = 'RL' THEN amount ELSE 0 END), 0),
        'cogs', COALESCE(SUM(CASE WHEN row_code = 'CS' THEN amount ELSE 0 END), 0),
        'gross_profit', COALESCE(SUM(CASE WHEN row_code = 'LB' THEN amount ELSE 0 END), 0),
        'opex', COALESCE(SUM(CASE WHEN row_code = 'DO' THEN amount ELSE 0 END), 0),
        'depreciation', COALESCE(SUM(CASE WHEN row_code = 'DA' THEN amount ELSE 0 END), 0),
        'ebitda', COALESCE(SUM(CASE WHEN row_code = 'EB' THEN amount ELSE 0 END), 0),
        'financial_result', COALESCE(SUM(CASE WHEN row_code = 'RF' THEN amount ELSE 0 END), 0),
        'other_income', COALESCE(SUM(CASE WHEN row_code = 'OR' THEN amount ELSE 0 END), 0),
        'other_expense', COALESCE(SUM(CASE WHEN row_code = 'OD' THEN amount ELSE 0 END), 0),
        'tax', COALESCE(SUM(CASE WHEN row_code = 'IR' THEN amount ELSE 0 END), 0),
        'net_result', COALESCE(SUM(CASE WHEN row_code = 'RL' THEN amount ELSE 0 END), 0),
        'margin_ebitda', CASE WHEN COALESCE(SUM(CASE WHEN row_code = 'RL' THEN amount ELSE 0 END), 0) <> 0
          THEN round(COALESCE(SUM(CASE WHEN row_code = 'EB' THEN amount ELSE 0 END), 0) / SUM(CASE WHEN row_code = 'RL' THEN amount ELSE 0 END) * 100, 2) ELSE 0 END,
        'margin_net', CASE WHEN COALESCE(SUM(CASE WHEN row_code = 'RL' THEN amount ELSE 0 END), 0) <> 0
          THEN round(COALESCE(SUM(CASE WHEN row_code = 'RL' THEN amount ELSE 0 END), 0) / SUM(CASE WHEN row_code = 'RL' THEN amount ELSE 0 END) * 100, 2) ELSE 0 END
