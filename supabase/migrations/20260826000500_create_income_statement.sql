-- ============================================================================
-- ETAPA 08E: DRE Gerencial — Demonstração do Resultado do Exercício
-- get_income_statement(p_from, p_to, p_cost_center_id, p_service_line_id)
--
-- Regime de competência via competence_date.
-- Deriva exclusivamente de financial_journal_entries + financial_journal_lines.
-- Cálculo: debit/credit + account nature (DEBITO → debit−credit, CREDITO → credit−debit).
-- 14 linhas DRE: Receita Bruta → Resultado Líquido.
-- Margens derivadas incluídas.
-- ============================================================================

-- ── 1. Revoke pre-existente ──────────────────────────────────────────────────

-- Function does not exist yet; grants/revokes are applied AFTER creation below.
...
AS $$
BEGIN
  -- Admin guard (CLI tests bypass via auth.uid() IS NULL)
...
-- DRE 14 linhas — preserves reference-app structure and terminology
...
COMMENT ON FUNCTION public.get_income_statement(
  date, date, uuid, uuid
) IS 'ETAPA 08E — DRE Gerencial. Regime de competência via competence_date. 14 linhas + margens. Ledger-only.';
