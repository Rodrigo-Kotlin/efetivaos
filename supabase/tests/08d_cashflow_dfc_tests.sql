-- ============================================================================
-- ETAPA 08D — SQL Test Suite: Cash Flow, 13-Week Projection, DFC (50 checks)
-- Execution: supabase db query --linked --file supabase/tests/08d_cashflow_dfc_tests.sql
...
INSERT INTO _test_results VALUES (5, 'realized_v has required columns',
  (SELECT count(*) FROM information_schema.columns WHERE table_schema='public' AND table_name='financial_cashflow_realized_v'
   AND column_name IN ('entry_id','transaction_id','entry_date','cash_effect','direction','dfc_class','amount','cash_accounts')) = 8, '');
...
-- ─── SECTION 10: Security grants (8) ──────────────────────────────────────

INSERT INTO _test_results VALUES (36, 'realized_v SELECT grant to authenticated',
...
INSERT INTO _test_results VALUES (37, 'forecast_v SELECT grant to authenticated',
  EXISTS (SELECT 1 FROM information_schema.role_table_grants
          WHERE table_schema='public' AND table_name='financial_cashflow_forecast_v'
          AND grantee='authenticated' AND privilege_type='SELECT'), '');

INSERT INTO _test_results VALUES (38, 'statement_v SELECT grant to authenticated',
  EXISTS (SELECT 1 FROM information_schema.role_table_grants
          WHERE table_schema='public' AND table_name='financial_cashflow_statement_v'
          AND grantee='authenticated' AND privilege_type='SELECT'), '');

INSERT INTO _test_results VALUES (39, 'realized_v SELECT grant to public',
  EXISTS (SELECT 1 FROM information_schema.role_table_grants
          WHERE table_schema='public' AND table_name='financial_cashflow_realized_v'
          AND grantee IN ('public','anon') AND privilege_type='SELECT'), '');

INSERT INTO _test_results VALUES (40, 'forecast_v SELECT grant to public',
  EXISTS (SELECT 1 FROM information_schema.role_table_grants
          WHERE table_schema='public' AND table_name='financial_cashflow_forecast_v'
          AND grantee IN ('public','anon') AND privilege_type='SELECT'), '');

INSERT INTO _test_results VALUES (41, 'statement_v SELECT grant to public',
  EXISTS (SELECT 1 FROM information_schema.role_table_grants
          WHERE table_schema='public' AND table_name='financial_cashflow_statement_v'
          AND grantee IN ('public','anon') AND privilege_type='SELECT'), '');

INSERT INTO _test_results VALUES (42, 'projection fn EXECUTE grant to authenticated',
...
INSERT INTO _test_results VALUES (43, 'projection fn EXECUTE grant to public',
  EXISTS (SELECT 1 FROM information_schema.role_routine_grants
          WHERE routine_schema='public' AND routine_name='cashflow_13_week_projection'
