...
type CatalogSourcingFilter = "all" | "own" | "outsourced";

function matchesStatus(active: boolean, filter: StatusFilter) {
  return filter === "all" || (filter === "active" ? active : !active);
}

function matchesSourcing(
  sourcing: CatalogItemRow["sourcing_type"],
  filter: CatalogSourcingFilter,
) {
  return (
    filter === "all" ||
    (filter === "own" ? sourcing === "own" : sourcing === "outsourced")
  );
}
...
export default function CatalogPage() {
  ...
  const [itemSourcing, setItemSourcing] =
  ...
  const [editingCategory, setEditingCategory] =
    useState<CatalogCategoryRow | null>(null);
  ...
  const approvedOwnItemIds = useMemo(() => {
    const ids = new Set<string>();
    for (const proposal of ownProposalsQuery.data ?? [])
      if (proposal.status === "approved") ids.add(proposal.catalog_item_id);
    return ids;
  }, [ownProposalsQuery.data]);
  ...
  const filteredItems = useMemo(
    () =>
      items.filter((item) => {
        ...
        return (
          matchesSearch &&
          (itemCategory === "all" || item.category_id === itemCategory) &&
          matchesSourcing(item.sourcing_type, itemSourcing) &&
          matchesStatus(item.active, itemStatus)
        );
      }),
    [itemCategory, itemSourcing, itemStatus, items, normalizedItemSearch],
  );
    ...
    () =>
      categories.filter((category) => {
        return (
          ...
          matchesStatus(category.active, categoryStatus)
        );
      }),
  ...
  async function saveCategory(input: CatalogCategoryInput) {
    if (!online) {
      const error = new Error(
        "Sem conexao. Reconecte para salvar a categoria.",
      );
      toast.error(error.message);
      throw error;
    }
    try {
      ...
      const duplicate = categories.find(
        (category) =>
          ...
          category.name.trim().toLocaleLowerCase("pt-BR") === normalizedName,
      );
      ...
      if (editingCategory) {
        if (
          ...
          !input.active &&
          !window.confirm(
            `Inativar a categoria "${editingCategory.name}"? Os itens historicos continuarao visiveis.`,
          )
        )
          return;
  ...
  async function changeItemStatus(item: CatalogItemRow) {
    ...
    if (
      item.active &&
      !window.confirm(
        `Inativar o item "${item.name}"? O historico sera preservado.`,
      )
    )
      return;
    try {
  ...
  async function changeCategoryStatus(category: CatalogCategoryRow) {
    ...
    if (
      category.active &&
      !window.confirm(
        `Inativar a categoria "${category.name}"? Os itens historicos continuarao visiveis.`,
      )
    )
      return;
    try {
      await setCategoryStatusMutation.mutateAsync({
        id: category.id,
  ...
  const isFirstLoad =
    activeQuery.isPending || (area === "items" && categoriesQuery.isPending);
  ...
  return (
    <div className="mx-auto max-w-[1480px]">
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button disabled={!online} onClick={openNewItem}>
      ...
      <div
        ...
        aria-label="Areas do catalogo"
      >
        <button
          type="button"
          aria-pressed={area === "items"}
          className={`rounded-lg px-5 py-2 text-sm font-bold transition-colors ${area === "items" ? "bg-emerald-800 text-white" : "text-slate-600 hover:bg-slate-50"}`}
          onClick={() => setArea("items")}
        >
          Itens
        </button>
        <button
          type="button"
          aria-pressed={area === "categories"}
          className={`rounded-lg px-5 py-2 text-sm font-bold transition-colors ${area === "categories" ? "bg-emerald-800 text-white" : "text-slate-600 hover:bg-slate-50"}`}
      ...
      {area === "items" ? (
        <section aria-label="Itens do catalogo">
          <div className="mb-4 grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-[minmax(240px,1fr)_220px_180px_180px]">
          ...
          {isFirstLoad ? (
          ...
          ) : activeQuery.isError || categoriesQuery.isError ? (
            <ErrorState
              onRetry={() => {
            ...
            <EmptyState
              ...
              description="Cadastre os itens e servicos que a Efetiva comercializa."
              action={
                <Button onClick={openNewItem}>
                  <Plus className="size-4" /> Cadastrar primeiro item
                </Button>
              }
            />
        ...
        <section aria-label="Categorias do catalogo">
          ...
          {isFirstLoad ? (
            ...
            <EmptyState
              ...
              description="Crie uma categoria antes de organizar os itens do catalogo."
              action={
            ...
            <EmptyState
              ...
              description="Ajuste a busca ou o filtro para ver outras categorias."
              action={
                <Button
                  variant="outline"
                  onClick={() => {
      ...
      <Drawer
        ...
        description="O codigo e gerado automaticamente; categoria e unidade completam a referencia canonica."
      >
        {categories.every((category) => !category.active) && (
          <div className="mb-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">
            Crie ou reative uma categoria para salvar um item.
          </div>
        )}
        <CatalogItemForm
          ...
          defaultValues={
            editingItem
              ? {
                  ...
                  sourcing_type: editingItem.sourcing_type,
      ...
      <Drawer
        ...
        description="Categorias organizam os itens sem remover referencias historicas."
      >
        <CatalogCategoryForm
          key={editingCategory?.id ?? "new-category"}
          defaultValues={
            editingCategory
              ? { name: editingCategory.name, active: editingCategory.active }
              : undefined
          }
          submitLabel={
