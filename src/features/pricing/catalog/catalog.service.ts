...
const itemSelect =
  "id, code, name, category_id, unit, sourcing_type, description, active, updated_at, category:catalog_categories!catalog_items_category_id_fkey(id, name, active)";
...
export function translateCatalogError(error: ServiceError): Error {
  const text =
    `${error.message ?? ""} ${error.details ?? ""} ${error.constraint ?? ""}`.toLowerCase();
  ...
  if (error.code === "23503") {
    return new Error(
      "A categoria selecionada nao existe mais ou possui registros relacionados.",
    );
  }
  if (error.code === "42501") {
  ...
  if (text.includes("categoria e unidade nao podem mudar")) {
    return new Error(
      "A categoria e a unidade nao podem ser alteradas porque o item ja participa do historico de cotacoes.",
    );
  }
  if (
    text.includes("a origem do item nao pode mudar depois que o item participa")
  ) {
    return new Error(
      "A origem do item nao pode ser alterada porque o item ja participa de cotacao, proposta de preco proprio ou precificacao vigente.",
    );
  }
  if (text.includes("codigo do item do catalogo nao pode ser alterado")) {
    return new Error(
      "O código do item é gerado automaticamente e não pode ser alterado.",
    );
  }
  if (text.includes("foreign key") || text.includes("violates foreign key")) {
    return new Error(
      "Nao foi possivel concluir porque o registro possui historico relacionado.",
    );
  }

  return new Error(
    "Nao foi possivel concluir a operacao no catalogo. Tente novamente.",
  );
}
...
export async function createCatalogItem(
  ...
  const payload: Database["public"]["Tables"]["catalog_items"]["Insert"] =
...
export async function updateCatalogItem(
  ...
  const payload: Database["public"]["Tables"]["catalog_items"]["Update"] =
    normalizeCatalogItemInput(input);
  const { data, error } = await supabase
    .from("catalog_items")
    .update(payload)
...
export async function setCatalogCategoryStatus(
  id: string,
  active: boolean,
): Promise<CatalogCategoryRow> {
  const { data, error } = await supabase
