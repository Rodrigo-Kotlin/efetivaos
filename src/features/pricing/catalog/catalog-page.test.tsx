...
vi.mock("../own-prices/own-prices-queries", () => ({
...
const items = [
  {
    ...
    unit: "exame",
    description: null,
    sourcing_type: "outsourced" as const,
    active: true,
    updated_at: "2026-08-23",
    category: { id: "category-active", name: "Laboratoriais", active: true },
  },
  {
    id: "item-inactive",
    code: "EXA-002",
    name: "Glicemia",
    category_id: "category-inactive",
    unit: "exame",
    description: null,
    sourcing_type: "outsourced" as const,
    active: false,
    updated_at: "2026-08-23",
    category: { id: "category-inactive", name: "Historica", active: false },
  },
  {
    id: "item-own",
    code: "PRP-001",
...
describe("CatalogPage", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(useOwnPriceProposals).mockReturnValue({
      data: [],
      isLoading: false,
      isError: false,
      refetch: vi.fn(),
    } as unknown as ReturnType<typeof useOwnPriceProposals>);
    vi.mocked(useCreateCatalogItem).mockReturnValue({
      mutateAsync: createItem,
      isPending: false,
    } as unknown as ReturnType<typeof useCreateCatalogItem>);
    vi.mocked(useUpdateCatalogItem).mockReturnValue({
      mutateAsync: vi.fn(),
      isPending: false,
    } as unknown as ReturnType<typeof useUpdateCatalogItem>);
    vi.mocked(useSetCatalogItemStatus).mockReturnValue({
      mutateAsync: setItemStatus,
      isPending: false,
    } as unknown as ReturnType<typeof useSetCatalogItemStatus>);
    vi.mocked(useCreateCatalogCategory).mockReturnValue({
      mutateAsync: createCategory,
      isPending: false,
    } as unknown as ReturnType<typeof useCreateCatalogCategory>);
    vi.mocked(useUpdateCatalogCategory).mockReturnValue({
      mutateAsync: vi.fn(),
      isPending: false,
    } as unknown as ReturnType<typeof useUpdateCatalogCategory>);
    vi.mocked(useSetCatalogCategoryStatus).mockReturnValue({
      mutateAsync: setCategoryStatus,
      isPending: false,
    } as unknown as ReturnType<typeof useSetCatalogCategoryStatus>);
    setItemStatus.mockReset().mockResolvedValue(undefined);
    setCategoryStatus.mockReset().mockResolvedValue(undefined);
    createItem
      .mockReset()
      .mockResolvedValue({ ...items[0], code: "ITEM-000001" });
  ...
  it("inativa com confirmacao e reativa item sem confirmacao", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Inativar item Hemograma" }),
    );
    expect(window.confirm).toHaveBeenCalledOnce();
    expect(setItemStatus).toHaveBeenCalledWith({
      id: "item-active",
      active: false,
    });
    ...
    await user.selectOptions(
      screen.getByRole("combobox", { name: "Filtrar itens por status" }),
      "all",
    );
    await user.click(
      screen.getByRole("button", { name: "Reativar item Glicemia" }),
    );
  ...
  it("inativa com confirmacao e reativa categoria sem confirmacao", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Inativar categoria Laboratoriais" }),
    );
    expect(window.confirm).toHaveBeenCalledOnce();
    expect(setCategoryStatus).toHaveBeenCalledWith({
      id: "category-active",
      active: false,
    });
    ...
    vi.mocked(window.confirm).mockClear();
    await user.selectOptions(
      screen.getByRole("combobox", { name: "Filtrar categorias por status" }),
      "all",
    );
    await user.click(
      screen.getByRole("button", { name: "Reativar categoria Historica" }),
    );
    expect(window.confirm).not.toHaveBeenCalled();
    expect(setCategoryStatus).toHaveBeenCalledWith({
      id: "category-inactive",
  ...
  it("exibe estado vazio e abre o drawer do primeiro item", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Seu catalogo ainda esta vazio" }),
    ).toBeInTheDocument();
    await user.click(
      screen.getByRole("button", { name: "Cadastrar primeiro item" }),
    );
    expect(
  ...
  it("aplica busca e apresenta estado filtrado vazio", async () => {
    ...
    renderPage();
    await user.type(
      screen.getByRole("textbox", { name: "Buscar itens" }),
      "inexistente",
    );
    expect(
  ...
  it("cria item sem enviar code e informa o codigo retornado pelo backend", async () => {
    ...
    expect(toast.success).toHaveBeenCalledWith(
      "Item ITEM-000001 cadastrado no catalogo.",
    );
  });
  ...
  it("filtra itens por origem propria e navega para precos proprios", async () => {
    const user = userEvent.setup();
    renderPage();

    await user.selectOptions(
      screen.getByRole("combobox", { name: "Filtrar itens por origem" }),
      "own",
    );
    expect(screen.queryByText("Hemograma")).not.toBeInTheDocument();
    expect(screen.getByText("Consulta propria")).toBeInTheDocument();
    expect(
      screen.getByRole("link", {
        name: /Definir preco proprio para Consulta propria/i,
      }),
    ).toHaveAttribute("href", "/pricing/own-prices");
  });

  it("identifica preco proprio aprovado e oferece consultar/propor reajuste", () => {
    vi.mocked(useOwnPriceProposals).mockReturnValue({
      data: [
        {
          id: "own-proposal-1",
          catalog_item_id: "item-own",
          status: "approved",
        },
      ],
      isLoading: false,
    ...
    expect(
      screen.getByRole("link", {
        name: /Consultar preco \/ propor reajuste para Consulta propria/i,
      }),
    ).toHaveAttribute("href", "/pricing/own-prices");
    expect(
