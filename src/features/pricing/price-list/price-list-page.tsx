...
import {
  ...
  formatRuleValue,
} from "@/features/pricing/comparison/comparison-helpers";
...
import type {
  CommercialOriginFilter,
  CommercialSourceFilter,
  CommercialStatusFilter,
  ComparisonRow,
} from "@/features/pricing/comparison/comparison-types";
import {
  EmptyState,
  ErrorState,
  PageHeader,
...
function SourceValidity({ validUntil }: { validUntil: string | null }) {
  ...
  return (
    <div>
      ...
      >
        {expired ? "Fonte vencida" : "Fonte vigente"}
      </p>
    </div>
  );
}
...
function PriceCard({
  ...
  onTrace: (row: ComparisonRow) => void;
}) {
  const reason = reviewReasonLabel(row.review_reason);
  const isOwn = row.price_origin === "own";
        ...
        <div>
          ...
          <dd>
            {isOwn
              ? isAdmin
                ? internalCostVisible
                  ? formatComparisonCurrency(internalCostVisible)
                  : "—"
                : "Restrito a Admin"
        ...
        <div>
          ...
          <dd>
            {isOwn ? (
              ...
              `${row.approved_supplier_name ?? "—"} · ${row.manual_source ? "Manual" : "Automatica"}`
            )}
          </dd>
        </div>
        <div>
        ...
        <div className="col-span-2">
          ...
          <dd className="mt-1">
            {isOwn ? (
              ...
              <SourceValidity validUntil={row.approved_source_valid_until} />
            )}
          </dd>
        </div>
      </dl>
...
export default function PriceListPage() {
  ...
  const ownProposalsQuery = useOwnPriceProposals();
  ...
  const [origin, setOrigin] = useState<CommercialOriginFilter>("all");
  ...
  const ownProposalItems = ownProposalsQuery.data ?? [];
  const ownProposalById = useMemo(() => {
    const map = new Map<string, OwnPriceProposalItem>();
    for (const proposal of ownProposalItems) map.set(proposal.id, proposal);
    return map;
  }, [ownProposalItems]);
  const internalCostFor = (row: ComparisonRow) =>
    ownProposalById.get(row.own_price_proposal_id ?? "")?.internal_cost ??
    null;
  const traceRow =
  ...
  const suppliers = useMemo(() => {
    ...
    for (const row of commercialRows)
      if (row.approved_supplier_id && row.approved_supplier_name)
        values.set(row.approved_supplier_id, row.approved_supplier_name);
    return Array.from(values, ([id, name]) => ({ id, name })).sort((a, b) =>
      a.name.localeCompare(b.name, "pt-BR"),
    );
  }, [commercialRows]);
  const filtered = commercialRows
    .filter((row) => {
      ...
      const matchesSource =
        ...
        (source === "own"
          ...
          : source === "manual"
            ? row.manual_source === true
            : row.manual_source === false);
      const matchesOrigin =
        origin === "all" ||
        (origin === "own"
          ? row.price_origin === "own"
      ...
      const matchesSupplier =
        supplier === "all" || row.approved_supplier_id === supplier;
      return (
        matchesSearch &&
        matchesCategory &&
        matchesStatus &&
    ...
    .sort((a, b) => {
      ...
      if (sortKey === "final_price")
        return (
          (Number(a.approved_final_price) - Number(b.approved_final_price)) *
          direction
        );
      if (sortKey === "approved_at")
        return (
          (a.approved_at ?? "").localeCompare(b.approved_at ?? "") * direction
        );
  ...
  return (
    <div className="mx-auto max-w-[1480px]">
      <PageHeader
        ...
        description="Uma linha comercial atual por item, com preco final aprovado, status e rastreabilidade da decisao."
        actions={
          <Button asChild variant="outline">
            <Link to="/pricing/comparison">Abrir comparacao</Link>
          </Button>
        }
      />
      <div className="mb-4 grid gap-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:grid-cols-2 xl:grid-cols-[minmax(16rem,1fr)_repeat(6,minmax(10rem,13rem))_auto]">
        ...
        <label>
          ...
          <select
            ...
            value={status}
            onChange={(event) =>
              setStatus(event.target.value as CommercialStatusFilter)
            }
          >
        ...
        <label>
          ...
          <select
            ...
            value={source}
            onChange={(event) =>
              setSource(event.target.value as CommercialSourceFilter)
            }
          >
        ...
        <label>
          ...
          <select
            ...
            value={origin}
            onChange={(event) =>
              setOrigin(event.target.value as CommercialOriginFilter)
            }
          >
            <option value="all">Origens: todas</option>
            <option value="own">Precos proprios</option>
        ...
        <label>
          ...
          <select
            ...
            value={sortKey}
            onChange={(event) =>
              setSortKey(event.target.value as PriceListSortKey)
            }
          >
            <option value="item">Ordenar: item</option>
            <option value="final_price">Ordenar: preco final</option>
            <option value="approved_at">Ordenar: aprovacao</option>
          </select>
        </label>
        <Button
          type="button"
      ...
      {query.isLoading || categoriesQuery.isLoading ? (
        ...
        <EmptyState
          title="Nenhum preco comercial aprovado"
          description="A tabela comercial e preenchida somente por uma aprovacao explicita de Admin na comparacao."
          action={
            <Button asChild>
              <Link to="/pricing/comparison">Ir para comparacao</Link>
            </Button>
          }
        />
      ) : filtered.length === 0 ? (
        <EmptyState
          title="Nenhum preco encontrado"
          description="Ajuste a busca ou os filtros para consultar outros precos."
          action={
          ...
          <div className="space-y-3 md:hidden">
            {filtered.map((row) => (
              <PriceCard
                ...
                row={row}
                isAdmin={isAdmin}
                internalCost={internalCostFor(row)}
                onTrace={(item) => setTraceItemId(item.catalog_item_id)}
              />
            ))}
          </div>
          <div className="hidden md:block">
            <TableShell>
              <table
                ...
                aria-label="Tabela de Precos"
              >
                <thead className="border-b border-slate-200 bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-500">
                  <tr>
                    <th className="px-4 py-3">Item</th>
                    <th className="px-4 py-3">Categoria</th>
                    <th className="px-4 py-3">Custo aprovado</th>
                    <th className="px-4 py-3">Fonte aprovada</th>
                    <th className="px-4 py-3">Validade da fonte</th>
                    <th className="px-4 py-3">Acrescimo</th>
                    ...
                    <tr key={row.catalog_item_id}>
                      ...
                      <td className="px-4 py-4">
                        {row.price_origin === "own" ? (
                          isAdmin ? (
                            internalCostFor(row) ? (
                              formatComparisonCurrency(internalCostFor(row))
                            ) : (
                              "—"
                            )
                          ) : (
                      ...
                      <td className="px-4 py-4">
                        {row.price_origin === "own" ? (
                          ...
                          <>
                            ...
                            <p className="text-xs text-slate-500">
                              {row.manual_source ? "Manual" : "Automatica"}
                            </p>
                          </>
                        )}
                      </td>
                      <td className="px-4 py-4">
                        {row.price_origin === "own" ? (
                          ...
                          <SourceValidity
                            validUntil={row.approved_source_valid_until}
                          />
                        )}
                      </td>
                      <td className="px-4 py-4">
                      ...
                      <td className="px-4 py-4 font-serif text-base font-bold text-emerald-950">
                        {formatComparisonCurrency(row.approved_final_price)}
                      </td>
                      <td className="px-4 py-4">
                        <CommercialStatusBadge status={row.effective_status} />
                      ...
                      <td className="px-4 py-4">
                        {formatComparisonDate(row.approved_at)}
                      </td>
                      <td className="px-4 py-4">
                        <Button
                          type="button"
                          size="sm"
                          variant="ghost"
                          onClick={() => setTraceItemId(row.catalog_item_id)}
                        >
                          Rastreabilidade
                        </Button>
                      </td>
      ...
      <ReviewDrawer
        ...
        online={online}
        ownProposal={
          traceRow?.price_origin === "own"
            ? ownProposalById.get(traceRow.own_price_proposal_id ?? "") ??
              null
            : null
        }
        onOpenChange={(open) => {
