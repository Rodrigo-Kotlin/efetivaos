...
import { Archive, ArchiveRestore, ArrowUpDown, Eye, Pencil, Plus, Search } from 'lucide-react'
...
function QuotationCard({ quotation, onArchive, onUnarchive, archivePending }: { quotation: QuotationListRow; onArchive: () => void; onUnarchive: () => void; archivePending: boolean }) {
  ...
  return <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
    ...
    <div className="mt-3 flex flex-col gap-2 sm:flex-row">
      <Button className="flex-1" asChild variant="outline"><Link to={`/pricing/quotations/${quotation.id}`}>{quotation.status === 'draft' && !isArchived ? <Pencil className="size-4" /> : <Eye className="size-4" />}{quotation.status === 'draft' && !isArchived ? 'Editar cotação' : 'Ver detalhes'}</Link></Button>
      {isArchived
        ? <Button type="button" variant="outline" size="sm" disabled={archivePending} onClick={onUnarchive}><ArchiveRestore className="size-4" /> Desarquivar</Button>
        : <Button type="button" variant="outline" size="sm" disabled={archivePending} onClick={onArchive}><Archive className="size-4" /> Arquivar</Button>}
    </div>
  </article>
}
...
export default function QuotationsPage() {
  ...
  const columns = useMemo<ColumnDef<QuotationListRow>[]>(() => [
    ...
    { accessorKey: 'updated_at', header: 'Atualizada', cell: ({ getValue }) => formatDateTime(getValue<string>()) },
    {
      ...
      cell: ({ row }) => <Button asChild variant="ghost" size="sm"><Link aria-label={`${row.original.status === 'draft' && !row.original.archived_at ? 'Editar' : 'Ver'} cotação ${row.original.reference_number || 'sem referência'}`} to={`/pricing/quotations/${row.original.id}`}>{row.original.status === 'draft' && !row.original.archived_at ? <Pencil className="size-4" /> : <Eye className="size-4" />}<span className="hidden xl:inline">{row.original.status === 'draft' && !row.original.archived_at ? 'Editar' : 'Detalhes'}</span></Link></Button>,
    },
  ], [])
  ...
  // Apply archive filter after TanStack filtering
  const filteredRows = useMemo(() => {
    const allRows = table.getRowModel().rows
    if (archiveFilter === 'all') return allRows
    if (archiveFilter === 'archived') return allRows.filter((row) => Boolean(row.original.archived_at))
    return allRows.filter((row) => !row.original.archived_at)
