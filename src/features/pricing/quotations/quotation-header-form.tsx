...
export function QuotationHeaderForm({ register, errors, suppliers, currentSupplierId, supplierWarning }: Props) {
  ...
  return <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <div className="mt-5 grid min-w-0 gap-5 sm:grid-cols-2">
      ...
      <div className="min-w-0"><label className="break-words text-sm font-semibold" htmlFor="received_at">Data recebida *</label><Input id="received_at" className="mt-1.5" type="date" {...errorA11y(Boolean(errors.received_at), 'received_at-error')} {...register('received_at')} /><FieldError id="received_at-error">{errors.received_at?.message}</FieldError></div>
      <div className="min-w-0"><label className="break-words text-sm font-semibold" htmlFor="valid_until">Validade</label><Input id="valid_until" className="mt-1.5" type="date" {...errorA11y(Boolean(errors.valid_until), 'valid_until-error')} {...register('valid_until')} /><FieldError id="valid_until-error">{errors.valid_until?.message}</FieldError></div>
      <div className="min-w-0 sm:col-span-2"><label className="break-words text-sm font-semibold" htmlFor="notes">Observações</label><textarea id="notes" className={`${textareaClassName} mt-1.5`} {...errorA11y(Boolean(errors.notes), 'notes-error')} {...register('notes')} /><FieldError id="notes-error">{errors.notes?.message}</FieldError></div>
    </div>
  </section>
}
