...
export default function DlpaPage() {
  const [dateFrom, setDateFrom] = useState('')
  const [dateTo, setDateTo] = useState('')

  const { data: rows, isLoading } = useDlpa(dateFrom || null, dateTo || null)
      ...
      {!isLoading && rows && (
        <div className="rounded-lg border border-slate-200 bg-white p-6">
          <div className="space-y-4">
            {rows.map((row, i) => {
              const isSaldo = row.row_label.includes('Saldo')
              const isResultado = row.row_label.includes('Resultado')
              return (
                <div
                  key={i}
                  className={`flex items-center justify-between py-3 ${
                    isSaldo ? 'border-t border-slate-200 font-semibold' : ''
                  } ${isResultado ? 'bg-blue-50 rounded px-3' : ''}`}
                >
                  <span className="text-slate-800">{row.row_label}</span>
                  <span className={`text-right ${isSaldo ? 'font-bold text-slate-900' : 'text-slate-700'}`}>
                    {fmt(row.amount)}
                  </span>
                </div>
              )
