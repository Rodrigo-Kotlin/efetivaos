...
import { transactionBaseSchema, type TransactionBaseFormValues, MOVEMENT_TYPE_LABELS, STATUS_LABELS } from '../schemas/finance-schemas'
...
function TransactionDetailDrawer({ transactionId, onClose }: { transactionId: string | null; onClose: () => void }) {
  ...
  const handleCancel = async () => {
    if (!tx) return
    if (!confirm('Tem certeza que deseja cancelar esta transacao?')) return
  ...
  return (
    <Drawer open={!!transactionId} onOpenChange={(o) => { if (!o) { onClose(); setSelectedEntryId(null) } }} title="Detalhes da Transacao">
      <div className="space-y-6">
        <div className="flex items-start justify-between">
          ...
          <Badge className={STATUS_COLORS[tx.status]}><StatusIcon className="mr-1 size-3" />{STATUS_LABELS[tx.status]}</Badge>
        </div>
            ...
            <div className="space-y-2">
              {entries.map((e) => (
                <button key={e.id} type="button" onClick={() => setSelectedEntryId(selectedEntryId === e.id ? null : e.id)}
                  ...
                  {e.review_required && <Badge className="mt-1 bg-orange-100 text-orange-800 text-xs">Revisao</Badge>}
                </button>
              ))}
            </div>
          )}
...
function TransactionCreateDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setFe({ _form: err?.message || 'Erro ao criar transacao.' })
    }
  }
  ...
  return (
    <Drawer open={open} onOpenChange={(o) => { if (!o) onClose() }} title="Nova Transacao">
      <div className="space-y-4 px-1">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="mb-1 block text-sm font-medium">Data Transacao *</label>
  ...
  return (
    <div className="mx-auto max-w-[1480px]">
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-serif text-3xl font-semibold tracking-tight">Transacoes</h1>
          <p className="mt-1 text-sm text-slate-600">{transactions.length} transacoes</p>
        </div>
        <Button onClick={() => setCreateOpen(true)}><Plus className="mr-2 size-4" />Nova Transacao</Button>
      </div>

      <div className="mb-6">
        <div className="relative max-w-md">
          <Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" />
          <Input className="pl-10" placeholder="Buscar transacao..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
      </div>
              ...
              {filtered.map(t => {
                ...
                return (
                  <tr key={t.id} className="border-b border-slate-50 hover:bg-slate-50/50 cursor-pointer" onClick={() => setDetailId(t.id)}>
                    ...
                    <td className="px-5 py-3"><Badge className={STATUS_COLORS[t.status]}><StatusIcon className="mr-1 size-3" />{STATUS_LABELS[t.status]}</Badge></td>
      ...
      <TransactionCreateDrawer open={createOpen} onClose={() => setCreateOpen(false)} />
